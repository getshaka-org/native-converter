 ...
 */
class JsObjectConstructionBenchmark:
  
  class C(val a: String, val b: Boolean, val c: Int) extends js.Object
  
  case class D(a: String, b: Boolean, c: Int, d: List[Set[String]]) derives NativeConverter
...
//    println(s"ms to make $randomJson objects with new js.Object + updateDynamic: ${end - start}")
    ...
    start = js.Date.now()
    while i < limit do
      res(i) = JSON.stringify(D("hello world", true, 69, List(Set("a", "b"), Set("c", "d"))).toNative)
      i += 1
    end = js.Date.now()
